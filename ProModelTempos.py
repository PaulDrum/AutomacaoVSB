...
Loop.Deletar(3, 1, max_row_ws+1, ws, "CANTEIRO TRADIMAQ 1")
Loop.Deletar(3, 1, max_row_ws+1, ws, "CANTEIRO TRADIMAQ 2")
...
for i in range(2, max_row_ws+1):
    ...
    aux = False
    for item in BaseMath:
        if ws.cell(i, 3).value == item[0]:
            ws.cell(i, 3).value = item[1]
            aux = True
            break
    if aux == False:
            ws.cell(i, 3).value = "---"
...
Loop.Deletar(3, 1, max_row_ws+1, ws, "---")

#INSERINDO VALOR NÚMERICO PARA CONTABILIZAR VIAGENS REPETIDAS
ws.cell(1, 5).value = "Qtd. Viagens"
for i in range(2, max_row_ws+1):
    if ws.cell(i, 2).value is None:
        break
    ws.cell(i, 5).value = 1
...
# acumulado_df = pd.pivot_table(df, values=['Qtd. Viagens'],index=['Macro Área Origem','Macro Área Destino','Área Origem','Área Destino','Tipo de viagem','Permanência no Destino (Horas)'],aggfunc='sum')
